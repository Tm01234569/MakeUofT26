...
String ArduinoGPTChat::sendImageMessage(const char* imageFilePath, String question) {
  ...
  String host;
  String path;
  if (!_extractHostAndPath(_apiUrl, &host, &path)) {
    Serial.println("Invalid API URL configuration");
    SPIFFS.remove(tempJsonFile);
  ...
  if (_tlsInsecure) {
    client.setInsecure(); // Optional: disable SSL certificate verification
  }
...
void ArduinoGPTChat::setChatModel(const char* model) {
  if (model != nullptr && strlen(model) > 0) {
    _chatModel = model;
  }
}

String ArduinoGPTChat::getChatModel() const {
  return _chatModel;
}

void ArduinoGPTChat::setTTSConfig(const char* model, const char* voice, const char* speed) {
  if (model != nullptr && strlen(model) > 0) {
    _ttsModel = model;
  }
  if (voice != nullptr && strlen(voice) > 0) {
    _ttsVoice = voice;
  }
  if (speed != nullptr && strlen(speed) > 0) {
    _ttsSpeed = speed;
  }
}

void ArduinoGPTChat::setTLSInsecure(bool insecure) {
  _tlsInsecure = insecure;
}

bool ArduinoGPTChat::isTLSInsecure() const {
  return _tlsInsecure;
}
...
bool ArduinoGPTChat::_extractHostAndPath(const String& url, String* host, String* path) const {
  if (host == nullptr || path == nullptr || url.length() == 0) {
    return false;
  }
...
String ArduinoGPTChat::_buildTTSPayload(String text) {
  text.replace("\"", "\\\"");
  return "{\"model\": \"" + _ttsModel + "\", \"input\": \"" + text + "\", \"voice\": \"" + _ttsVoice + "\"}";
}
